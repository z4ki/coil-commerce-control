...
const formSchema = z.object({
  ...
  method: z.enum(paymentFormMethods),
  notes: z.string().optional(),
  checkNumber: z.string().optional(),
}).superRefine((data, ctx) => {
    if (data.method === 'check' && (!data.checkNumber || data.checkNumber.trim() === '')) {
        ctx.addIssue({
...
export const PaymentForm = ({ saleId, payment, onSuccess, onCancel }: PaymentFormProps) => {
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      amount: payment?.amount || Math.max(0, remainingAmountForSale),
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        {form.watch('method') === 'check' && (
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t('form.payment.checkNumber') || 'Check Number'}</FormLabel>
                <FormControl>
